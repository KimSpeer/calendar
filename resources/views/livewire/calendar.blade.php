                                ...
                                @foreach ($daysinmonth as $day)
                                    @if (substr($day, 0, 3) == 'pre')
                                        ...
                                        @elseif(substr($day, 8, 10) == substr($today, 0, 10))
                                            <div
                                                class="relative h-16 px-3 py-2 font-semibold text-indigo-600 bg-white ">
                                                <time
                                                    class="flex items-center justify-center w-6 h-6 font-semibold text-white bg-indigo-600 rounded-full"
                                                    datetime="{{ substr($day, 8, 11) }}">{{ substr($day, 16, 2) }}</time>
                                            @else
                                                <div class="relative h-16 px-3 py-2 text-gray-500 bg-white h-15">
                                                    <time
                                                        datetime="{{ substr($day, 8, 11) }}">{{ substr($day, 16, 2) }}</time>
                                    @endif
                                    ...
                                    @if (isset($nom))
                                        <ol class="mt-2">
                                            @foreach ($nom as $n)
                                                @if (substr($day, 8, 10) == $n->eventStart)
                                                    <li>
                                                        <a href="#" class="flex group">
                                                            <p
                                                                class="flex-auto font-medium text-gray-900 truncate group-hover:text-indigo-600">
                                                            ...
                                                            <time datetime="{{ $n->eventStart }}"
                                                                class="flex-none hidden ml-3 text-gray-500 group-hover:text-indigo-600 xl:block">{{ $n->eventStart }}</time>
                                                        </a>
                                                    </li>
                                                @endif
                                            @endforeach
